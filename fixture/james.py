...
class JamesHelper:
    ...
    class Session:
        ...
        # encode str to bytes for telnet.read_until method
        def read_until(self, text):
            self.telnet.read_until(text.encode("ascii"), 5)

        # encode str to bytes for telnet.write method
        def write(self, text):
            self.telnet.write(text.encode("ascii"))

        def is_user_registered(self, username):
            self.write(f"verify {username}\n")
            # res - tuple whith 3 elements: 1-number of matching, 2-regular expression, 3-read text
